...
export default function SummaryScreen() {
  ...
  const fetchOnboardingData = async () => {
    try {
      ...
      const combinedData = {
        ...onboardingData,
        id: session.user.id,
        full_name: onboardingData?.full_name || user.user_metadata?.full_name || user.email?.split('@')[0],
        email: user.email,
  ...
  // Debug logging
  console.log('Weight data:', onboardingData.weight);
  console.log('Height data:', onboardingData.height);
            ...
            <View style={styles.infoSection}>
              ...
              <View style={styles.infoRow}>
                ...
                <View style={styles.infoTextContainer}>
                  ...
                  <Text style={styles.infoValue}>
                    {displayHeight ? `${displayHeight} ${useImperial ? 'in' : 'cm'}` : 'Not set'}
                    </Text>
                    <Text style = {styles.warningText}>
                    {displayHeight < 40 ? ' please consider raising your height it might be unrealistically low' : ''}
                    {displayHeight > 100 ? ' please consider lowering your height it might be unrealistically high' : ''}
              ...
              <View style={styles.infoRow}>
                ...
                <View style={styles.infoTextContainer}>
                  ...
                  <Text style={styles.infoValue}>
                    {displayWeight ? `${displayWeight} ${useImperial ? 'lbs' : 'kg'}` : 'Not set'}
                  
            
                  </Text>
                  <Text style = {styles.warningText}>
                    {displayWeight && useImperial && displayWeight < 60 ? ' please consider raising your weight it might be unrealistically low' : ''}
                    {displayWeight && !useImperial && displayWeight < 27 ? ' please consider raising your weight it might be unrealistically low' : ''}
                  </Text>
                </View>
              </View>

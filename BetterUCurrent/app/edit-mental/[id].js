...
export default function EditMentalScreen() {
  ...
  const [calmnessLevel, setCalmnessLevel] = useState('');
  ...
  const fetchSession = async () => {
    try {
      ...
      setCalmnessLevel(data.calmness_level?.toString() || '');
  ...
  const handlePhotoUpload = async () => {
    try {
      ...
      const fileName = isVideo ? 'mental.mp4' : 'mental.jpg';
      ...
      formData.append('file', {
        ...
        name: fileName, // 'mental.mp4' or 'mental.jpg'
      });
  ...
  const handleSave = async () => {
    ...
    try {
      const updates = {
        ...
        calmness_level: calmnessLevel ? parseInt(calmnessLevel) : null,
  ...
  const handleRemoveColor = async () => {
    Alert.alert(
      'Remove Border Color?',
      'This will remove the custom border from your session post.',
      [
        { text: 'Cancel', style: 'cancel' },
        {
          text: 'Remove',
  ...
  const handleDelete = () => {
    Alert.alert(
      'Delete session?',
      'This will permanently remove this mental session from your history.',
      [
        { text: 'Cancel', style: 'cancel' },
        {
          text: 'Delete',
          ...
          onPress: async () => {
            try {
              ...
              const { error } = await supabase
                .from('mental_session_logs')
                .delete()
                .eq('id', id); // Ensure only this session is removed.
              if (error) throw error;
              // Reset cached feed so the deleted mental session disappears without waiting for a manual refresh.
              setFeedLoaded(false);
              setCachedFeedData({
                feed: [],
                allFeedItems: [],
  ...
  return (
    ...
    >
      <View style={styles.header}>
        ...
        <Text style={styles.title}>Edit Mental Session</Text>
      </View>
      ...
      <View style={styles.form}>
        ...
        <View style={styles.row}>
          ...
          <View style={[styles.inputGroup, { flex: 1, marginLeft: 8 }]}>
            <Text style={styles.label}>Calmness Level (1-5)</Text>
            <TextInput
              style={styles.input}
              value={calmnessLevel}
              onChangeText={setCalmnessLevel}
              placeholder="0"
              placeholderTextColor="#666"
              keyboardType="numeric"
              maxLength={1}
        ...
        <View style={styles.inputGroup}>
          ...
          <TextInput
            ...
            placeholder="e.g., Meditation, Breathing"
            placeholderTextColor="#666"
          />
        </View>
        ...
        <View style={styles.borderColorSection}>
          ...
          <Text style={styles.sectionDescription}>
            Make your session stand out in the feed with a custom colored border!
          </Text>
        ...
        <View style={styles.photoSection}>
          <Text style={styles.sectionTitle}>Session Photo</Text>
          <TouchableOpacity 
        ...
        >
          {deleting ? (
            ...
            <Text style={styles.deleteActivityText}>Delete Session</Text>
          )}
        </TouchableOpacity>
      </View>
    </ScrollView>

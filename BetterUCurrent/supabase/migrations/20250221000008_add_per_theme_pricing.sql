-- Add Per-Theme Pricing Support
-- Allows themes to have custom prices that override rarity-based pricing
-- If theme_bank.neuros_cost is NULL, falls back to rarity_pricing

-- ============================================================================
-- 1. MAKE THEME_BANK.NEUROS_COST NULLABLE
-- ============================================================================
-- Change from NOT NULL DEFAULT 0 to nullable, so NULL means "use rarity pricing"
ALTER TABLE public.theme_bank 
  ALTER COLUMN neuros_cost DROP NOT NULL,
  ALTER COLUMN neuros_cost DROP DEFAULT;

-- Update comment to explain the new behavior
COMMENT ON COLUMN public.theme_bank.neuros_cost IS 
  'Custom price for this theme. If NULL, uses rarity-based pricing from rarity_pricing table. If set, overrides rarity pricing.';

-- ============================================================================
-- 2. UPDATE ASSIGNMENT FUNCTION TO USE PER-THEME PRICING
-- ============================================================================
-- Modified to check theme_bank.neuros_cost first, then fall back to rarity_pricing
...
DECLARE
  ...
  v_theme_custom_cost INTEGER; -- Custom price from theme_bank
...
COMMENT ON FUNCTION public.assign_user_theme_slots(UUID, UUID) IS 
  'Assigns 5 themes to a user for a rotation with randomized rarities. Uses per-theme pricing from theme_bank.neuros_cost if set, otherwise falls back to rarity-based pricing from rarity_pricing. Rarity distribution: 50% common, 30% rare, 15% epic, 4.9% legendary, 0.1% mythic.';
